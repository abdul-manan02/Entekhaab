...
const updateRequest = async (req, res) => {
  try {
    ...
    const request = await partyApproval.findOneAndUpdate(
      { _id: id },
      { status },
      { new: true, runValidators: true }
    );
    if (!request) return res.status(404).json({ msg: "No request found" });
    if (status === "Accepted") {
      console.log("in accepted");
      const response = await axios.put(
        `http://localhost:1003/api/v1/party/name/${name}/approval`
      );
...
export { getAllRequests, getParty, createRequest, updateRequest };
