...
const getPartyRequests = async (req, res) => {
    try {
        ...
        const memberApproval = await MemberApproval.find({partyId});
        res.status(200).json(memberApproval);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
const getPendingPartyRequests = async (req, res) => {
    try {
        const {partyId} = req.params;
        const memberApproval = await MemberApproval.find({partyId, status: "Pending"});
        res.status(200).json(memberApproval);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
const updateApproval = async (req, res) => {
    try {
        ...
        if(status=="Rejected"){
            ...
            const partyResponse = await axios.patch(partyEndpoint, { action:"Add", memberID: memberApproval.memberId.toString(), token },{
                headers: {
